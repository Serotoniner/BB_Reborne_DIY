...
<#
    ...
    Noir map textures wrapper for the BBReborne Noir tab / workflow.
    ...
    Notes:
    - This wrapper lives under Scripts\Noir\Mapfiles.
    - The actual worker scripts live under Scripts\Noir\Textures and are called by this wrapper.
...
function Invoke-TexturePipeline {
    ...
    try {
        ...
        $diffuseArgs = $commonArgs + @('-RealEsrganExe', $RealEsrganExe, '-ThrottleLimit', $GpuThrottleLimit) + $realEsrganModelArgs + $applyArgs + $keepArgs + $noUpscaleArgs + $noirArgs
...
$noirRoot = Split-Path -Parent $PSScriptRoot
$texturesScriptRoot = Join-Path $noirRoot 'Textures'
...
if ([string]::IsNullOrWhiteSpace($TexturesRoot)) {
    ...
    $TexturesRoot = Join-Path (Join-Path (Join-Path $OutputRoot 'BBReborne_noir_textures') 'map') ''
}

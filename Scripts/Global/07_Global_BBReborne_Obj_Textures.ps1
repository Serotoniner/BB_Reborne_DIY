...
<#
    ...
    - If <OutputRoot>\BBReborne_obj already contains a patched/copy of the same object, use that
      patched binder as the per-object source so earlier Global OBJ diff patches are preserved.
    ...
    - Write final object binders under <OutputRoot>\BBReborne_obj\obj.
...
function Build-ObjectJobs {
    ...
    #   <OutputRoot>\BBReborne_obj\obj\*.objbnd.dcx
...
if (-not $DiffuseScript) { $DiffuseScript = Join-Path $texturesScriptRoot '01_Mapfiles_BBReborne_Textures_Upscale_Diffuse_2x_AI.ps1' }
if (-not $LinearScript) { $LinearScript = Join-Path $texturesScriptRoot '02_Mapfiles_BBReborne_Textures_Upscale_Data_Linear_RGBA_BC1.ps1' }
if (-not $ReflectanceScript) { $ReflectanceScript = Join-Path $texturesScriptRoot '04_Mapfiles_BBReborne_Textures_Reflectance_Fix.ps1' }
if (-not $SpecularScript) { $SpecularScript = Join-Path $texturesScriptRoot '05_Mapfiles_BBReborne_Textures_Specular_Fix.ps1' }
...
$script:RunWorkDir = Join-Path (Join-Path $OutputRoot '_work') "obj_textures_$stamp"
...
try {
    ...
    $jobs = @(Build-ObjectJobs -SourceRoot $SourceObjRoot -FinalRoot $objOutputRoot -AllowedIds $allowedIds -OverlayRoot $objOutputRoot -OverlayAllowedIds $diffBackedOverlayIds)
    ...
    if ($diffuseManifest.Count -gt 0) {
        ...
        Invoke-PwshTextureScript `
            ...
            -Args ($commonArgs + $realEsrganArgs + $textureScriptExtraArgs + $noUpscaleArgs)
    } else {

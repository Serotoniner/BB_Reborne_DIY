...
<#
    03b_Mapfiles_BBReborne_Maplight_BTL_Development.ps1

    BTL JSON patch generator for the BBReborne map tab.
    ...
    - Patch output is staged under <NewDir>\files so it does not share the real BTL map folders.
...
function Get-BtlJsonRelFromDcxRel([string]$Rel) {
...
function Get-PatchDirForRel([string]$Rel) {
    ...
    #   <NewDir>\files\m21_00_00_00\_patches\m21_00_00_00_0000.btl.patch
...
function Get-PatchLeafForJsonRel([string]$JsonRel) {
    # Keep the typed extension in the patch filename so BTL and future BTPB patches
    # can coexist in the same map _patches folder:
    #   m21_00_00_00_0000.btl.patch
    #   m21_00_00_00_0000.btpb.patch
    $leaf = Split-Path -Leaf (($JsonRel -replace '\\','/'))
    $leaf = ($leaf -replace '\.btl\.json$', '.btl.patch')
...
function Export-RowsByPatchDir {
    ...
    # folders hold only the individual .btl.patch files.
    Ensure-Dir $outRoot
...
function Clear-StaleBtlPatchOutputsForRows {
    param([Parameter(Mandatory=$true)][object[]]$Rows)

    # Do not remove any real source folders. Only remove previous generated BTL
    # patch artifacts from each map _patches folder and the root aggregate patch.
    $rootAllPatch = Join-Path $outRoot "all_changed_btl_json.patch"
    ...
    foreach ($pd in $patchDirs) {
        Ensure-Dir $pd
        Get-ChildItem -LiteralPath $pd -File -Filter "*.btl.patch" -ErrorAction SilentlyContinue | Remove-Item -Force
    }
}

function Get-BtlIndex([string]$Root, [bool]$DoRecurse, [string]$RegexText, [string[]]$ExcludeTopFolders) {
    $gciParams = @{
        LiteralPath = $Root
        Filter      = "*.btl.dcx"
        File        = $true
...
# BTL development patch generation uses WitchyBND v2.14.4.5.
...
try {
    ...
    # ------------------------------------------------------------
    Write-Host ""
    Write-Host "Stage #0: discovering and hashing BTL DCX files..."

    $excludeTop = @($PatchDirName, $WorkDirName, "files")
    $oldIndex = Get-BtlIndex -Root $oldRoot -DoRecurse ([bool]$Recurse) -RegexText $IncludeRegex -ExcludeTopFolders $excludeTop
    $newIndex = Get-BtlIndex -Root $newRoot -DoRecurse ([bool]$Recurse) -RegexText $IncludeRegex -ExcludeTopFolders $excludeTop
    $allKeys = @($oldIndex.Keys + $newIndex.Keys | Sort-Object -Unique)

    if ($StartAt -gt 0) {
        if ($StartAt -ge $allKeys.Count) { throw "-StartAt $StartAt >= BTL count $($allKeys.Count)" }
    ...
    if ($allKeys.Count -eq 0) {
        Write-Host "No *.btl.dcx files found after filters."
        $success = $true
        return
    }
    ...
    foreach ($key in $allKeys) {
        ...
        if (($n % 25) -eq 0 -or $n -eq $allKeys.Count) {
            Write-Progress -Activity "Hash BTL DCX files" -Status ("{0}/{1}" -f $n, $allKeys.Count) -PercentComplete ([int](100.0 * $n / $allKeys.Count))
        }
        ...
        [void]$compareRows.Add([pscustomobject]@{
            BtlBase = Get-BtlBaseFromLeaf (Split-Path -Leaf $rel)
            Rel     = $rel
            JsonRel = Get-BtlJsonRelFromDcxRel $rel
            Status  = $status
            OldPath = $oldPath
            NewPath = $newPath
            OldSize = $oldSize
    ...
    Write-Progress -Activity "Hash BTL DCX files" -Completed

    if (-not $DryRun) {
        Export-RowsByPatchDir -Rows @($compareRows) -FileName "compare_btls.csv" -RelProperty "Rel"
    }
    ...
    Write-Host ("BTL DCX compare done. Same={0} Changed={1} Added={2} Removed={3}" -f $sameCount, $changedCount, $addedCount, $removedCount)
    if (-not $DryRun) { Write-Host ("Compare CSV: {0}" -f (Join-Path $outRoot "compare_btls.csv")) }
    ...
    if ($nonPatchable.Count -gt 0) {
        Write-Host ("  Note: Added/Removed BTLs are listed in compare_btls.csv but skipped for JSON patch export: {0}" -f $nonPatchable.Count) -ForegroundColor Yellow
    }


    if (-not $DryRun) {
        Clear-StaleBtlPatchOutputsForRows -Rows @($compareRows)
    }

    if ($changedRows.Count -eq 0) {
        Write-Host "No changed matching BTL DCX files. Nothing to decompress or patch."
        $success = $true
        return
    }
    ...
    # ------------------------------------------------------------
    # Stage #1: copy changed BTL DCX files into work/a and work/b
    # ------------------------------------------------------------
    Write-Host ""
    Write-Host ("Stage #1: copying {0} changed BTL DCX pairs into work..." -f $changedRows.Count)
    ...
    # ------------------------------------------------------------
    # Stage #3: dump BTL JSON
    # ------------------------------------------------------------
    Write-Host ""
    Write-Host ("Stage #3: dumping BTL JSON, throttle={0}..." -f $DumpThrottle)
    ...
    foreach ($r in $workRows) {
        [void]$dumpItems.Add([pscustomobject]@{ BtlBase=$r.BtlBase; Rel=$r.Rel; JsonRel=$r.JsonRel; Side="old"; Btl=$r.OldBtl; Json=$r.OldJson })
        [void]$dumpItems.Add([pscustomobject]@{ BtlBase=$r.BtlBase; Rel=$r.Rel; JsonRel=$r.JsonRel; Side="new"; Btl=$r.NewBtl; Json=$r.NewJson })
    }

    # Keep this stage deliberately simple and robust. BTL files are few, and using a captured
    ...
    foreach ($it in $dumpItems) {
        $dumpN++
        Write-Progress -Activity "Dump BTL JSON" -Status ("{0}/{1}: {2} [{3}]" -f $dumpN, $dumpItems.Count, $it.Rel, $it.Side) -PercentComplete ([int](100.0 * $dumpN / $dumpItems.Count))
    ...
    if ($dumpFailures.Count -gt 0) {
        ...
        if ($FailOnDumpFailure) { throw "BTL JSON dump failed for $($dumpFailures.Count) file(s). See: $dumpManifest" }
        $KeepWork = $true
        Write-Host "  Continuing with BTLs whose old and new JSON both dumped successfully. Work folder will be kept." -ForegroundColor Yellow
    }
    else {
    ...
    if ($jsonChangedRows.Count -eq 0) {
        ...
        if ($dumpFailures.Count -gt 0) {
            Write-Host "No patches were generated from successful dumps; one or more BTLs failed to dump and are marked DumpFailed." -ForegroundColor Yellow
        } else {
            Write-Host "All changed BTL DCX files produced identical BTL JSON. No patch files were generated."
        }
        Write-Host "JSON manifest: $jsonManifestPre"
        $success = $true
        return
    }
    Write-Host ("  JSON-changing BTLs: {0}" -f $jsonChangedRows.Count)

    # ------------------------------------------------------------
    # Stage #5: export portable Git patches
    # ------------------------------------------------------------
    Write-Host ""
    Write-Host ("Stage #5: exporting Git patches for {0} JSON-changing BTL(s)..." -f $jsonChangedRows.Count)
    ...
    foreach ($r in $jsonChangedRows) {
        ...
        if (-not (Test-Path -LiteralPath $r.OldJson -PathType Leaf) -or -not (Test-Path -LiteralPath $r.NewJson -PathType Leaf)) {
            [void]$patchExportFailures.Add([pscustomobject]@{ BtlBase=$r.BtlBase; JsonRel=$r.JsonRel; Error="Missing old/new JSON file for direct diff" })
            continue
        }
        ...
        if ($code -eq 1) {
            ...
            $allPatch = Join-Path $outRoot "all_changed_btl_json.patch"
        ...
        elseif ($code -eq 0) {
            Write-Host ("  WARN: {0} JSON hashes differed, but git diff produced no patch." -f $r.BtlBase) -ForegroundColor Yellow
        }
        else {
            $first = ($diffText | Select-Object -First 1)
            [void]$patchExportFailures.Add([pscustomobject]@{ BtlBase=$r.BtlBase; JsonRel=$r.JsonRel; Error="git diff failed with exit code $code. $first" })
        }
    }
    ...
    if ($patchExportFailures.Count -gt 0) {
        ...
        throw "Patch export failed for $($patchExportFailures.Count) BTL(s). See: $failCsv"
    }
    ...
    Write-Host ("BTL DCX changed pairs: {0}" -f $changedRows.Count)
    Write-Host ("JSON-changing BTLs:   {0}" -f $jsonChangedRows.Count)

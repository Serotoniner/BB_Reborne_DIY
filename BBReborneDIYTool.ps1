...
function Load-PathFiles {
    ...
    try {
        ...
        if ($config.toolRoot) { $TxtToolRoot.Text = [string]$config.toolRoot }
...
function Save-PathFiles {
    ...
    $lines.Add('#   . "C:\Path\To\BBReborneDIYTool.paths.ps1"')
    $lines.Add('')
    ...
    $lines.Add('$BBR_ToolRoot = ' + (Quote-PSString $config.toolRoot))
    ...
    foreach ($tool in $Tools) {
        $exe = $RowControls[$tool.Key].Exe.Text.Trim()
        $lines.Add('    ' + (Quote-PSString $tool.Label) + ' = ' + (Quote-PSString $exe))
    }
    $lines.Add('}')
